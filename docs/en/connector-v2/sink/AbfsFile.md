...
> Azure Blob File System (ABFS) file sink connector
...
|---------------------------------------|---------|----------|--------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| path                                  | string  | yes      | -                                          | The ABFS path to write file in.                                                                                                                                                 |
| tmp_path                              | string  | no       | /tmp/seatunnel                             | The result file will write to a tmp path first and then use `mv` to submit tmp dir to target dir. Need an ABFS dir.                                                             |
...
Azure storage endpoint. Default is `dfs.core.windows.net`.
...
sink {
  AbfsFile {
    path = "abfs://mycontainer@myaccount.dfs.core.windows.net/seatunnel/sink"
...
sink {
  AbfsFile {
    ...
    container = "mycontainer"
    path = "abfs://mycontainer@myaccount.dfs.core.windows.net/tmp/fake_empty/text/${table_name}"
    row_delimiter = "\n"
    partition_dir_expression = "${k0}=${v0}"
    is_partition_field_write_in_file = true
    file_name_expression = "${transactionId}_${now}"
