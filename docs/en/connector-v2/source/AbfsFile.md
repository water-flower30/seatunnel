import ChangeLog from '../changelog/connector-file-abfs.md';

# AbfsFile

> Azure Blob File System (ABFS) file source connector
...
1. You must ensure your spark/flink cluster already integrated hadoop. The tested hadoop version is 2.x/3.x.
2. You must ensure `hadoop-azure-xx.jar` and `azure-storage-xx.jar` in `${SEATUNNEL_HOME}/plugins/` dir and the version of `hadoop-azure` jar need equals your hadoop version which used in spark/flink and `azure-storage-xx.jar` version needs to be the version corresponding to the `hadoop-azure` version. Eg: `hadoop-azure-3.1.4.jar` dependency `azure-storage-8.6.6.jar`.

### For SeaTunnel Zeta Engine

1. You must ensure `seatunnel-hadoop3-3.1.4-uber.jar`, `azure-storage-8.6.6.jar`, and `hadoop-azure-3.1.4.jar` in `${SEATUNNEL_HOME}/lib/` dir.
...
|----------------------------|---------|----------|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| path                       | string  | yes      | -                   | The ABFS path that needs to be read can have sub paths, but the sub paths need to meet certain format requirements. Specific requirements can be referred to "parse_partition_from_path" option                                                                                                                                     |
...
| container                  | string  | yes      | -                   | Azure storage container name                                                                                                                                                                                                                                                                                                        |
| endpoint                   | string  | no       | dfs.core.windows.net | Azure storage endpoint                                                                                                                                                                                                                                                                                                              |
...
## How to Create an ABFS Data Synchronization Jobs

The following example demonstrates how to create a data synchronization job that reads data from ABFS and prints it on the local client:
...
# Create a source to connect to ABFS
source {
  AbfsFile {
    path = "abfs://mycontainer@myaccount.dfs.core.windows.net/seatunnel/orc"
...
# Console printing of the read ABFS data
sink {
  Console {
  }
}
...
source {
  AbfsFile {
    path = "abfs://mycontainer@myaccount.dfs.core.windows.net/seatunnel/json"
    account_name = "myaccount"
    account_key = "xxxxxxxxxxxxxxxxx"
    container = "mycontainer"
    ...
    tables_configs = [
      {
          ...
          container = "mycontainer"
          path = "abfs://mycontainer@myaccount.dfs.core.windows.net/test/seatunnel/read/orc"
          file_format_type = "orc"
      },
      {
          schema = {
    ...
    tables_configs = [
      {
          ...
          account_key = "xxxxxxxxxxxxxxxxxxx"
          container = "mycontainer"
          path = "abfs://mycontainer@myaccount.dfs.core.windows.net/test/seatunnel/read/json"
          file_format_type = "json"
          schema = {
            table = "fake01"
            fields {
